...
-- Compatibility/safety layer for DEVIL HUB
local function getChild(parent, name, timeout)
    if not parent then return nil end
    local ok, obj = pcall(function() return parent:FindFirstChild(name) end)
    if ok and obj then return obj end
    if timeout and timeout > 0 then
        local ok2, waited = pcall(function() return parent:WaitForChild(name, timeout) end)
        if ok2 then return waited end
    end
    return nil
end
...
local function fireRemote(remote, ...)
    ...
    if not ok then
        local key = tostring(remote:GetFullName())
        if not _remoteWarned[key] then
            _remoteWarned[key] = true
            warn("[DEVIL HUB] Remote failed: " .. key .. " | " .. tostring(err))
        end
    end
    return ok
end

local function getEquipPetEvent()
    local events = getChild(ReplicatedStorage, "rEvents", 5)
    local remote = getChild(events, "equipPetEvent", 5)
    if remote then return remote end
    local ok, found = pcall(function()
        return ReplicatedStorage:FindFirstChild("equipPetEvent", true)
    end)
    return ok and found or nil
end

local function getMuscleEvent()
    local remote = getChild(LocalPlayer, "muscleEvent", 5)
    if remote then return remote end
    local character = LocalPlayer.Character
    remote = getChild(character, "muscleEvent", 2)
    if remote then return remote end
    local backpack = getChild(LocalPlayer, "Backpack", 2)
    remote = getChild(backpack, "muscleEvent", 2)
    if remote then return remote end
    local ok, found = pcall(function()
        return ReplicatedStorage:FindFirstChild("muscleEvent", true)
    end)
    return ok and found or nil
end


-- Anti-AFK compatibility
do
    local VirtualUser = game:GetService("VirtualUser")
    pcall(function()
...
local dropdown = Killer:AddDropdown("Select Pet", function(text)
    local petsFolder = getChild(LocalPlayer, "petsFolder", 5)
    local remote = getEquipPetEvent()
    if not petsFolder or not remote then return end
    for _, folder in pairs(petsFolder:GetChildren()) do
        if folder:IsA("Folder") then
            for _, pet in pairs(folder:GetChildren()) do
                fireRemote(remote, "unequipPet", pet)
            end
        end
    end
    task.wait(0.2)
    ...
    local petsToEquip = {}
    local unique = getChild(petsFolder, "Unique", 5)
    ...
    for i = 1, equippedCount do
        fireRemote(remote, "equipPet", petsToEquip[i])
        task.wait(0.1)
    end
end)
    ...
    task.spawn(function()
        while autoPunchNoAnim do
            ...
            if punch then
                ...
                fireRemote(getMuscleEvent(), "punch", "rightHand")
                fireRemote(getMuscleEvent(), "punch", "leftHand")
            else
        ...
        task.spawn(function()
            while godDamageActive do
                ...
                if groundSlam then
                    ...
                    fireRemote(getMuscleEvent(), "slam")
                    groundSlam:Activate()
                end
                task.wait(0.1)
            end
...
local function gettool()
    ...
    game:GetService("Players").fireRemote(getMuscleEvent(), "punch", "leftHand")
    game:GetService("Players").fireRemote(getMuscleEvent(), "punch", "rightHand")
end
